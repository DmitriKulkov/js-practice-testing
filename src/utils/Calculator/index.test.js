...
const test_sum = (message, a,b)=>test(message, ()=>{
  expect(sum(a,b)).toBe(a+b)
})

const test_subtrasct = (message, a, b) => test(message, ()=>{
  expect(subtract(a,b)).toBe(a-b)
})

const test_multiply = (message, a, b) => test(message, ()=>{
  expect(multiply(a,b)).toBe(a*b)
})

const test_divide = (message, a, b) => test(message, ()=>{
  expect(divide(a,b)).toBe(b===0?NaN:a/b)
})

const test_divide_by_zero = (a,b) => test('display division by zero error',()=>{
  const logSpy = jest.spyOn(console, 'log');
  divide(a,b)
  expect(logSpy).toHaveBeenCalledWith('division by zero');
})
