...
function App() {
  ...
  */
   ...
   */
  const getFirstArgumentAsString = () => {
   ...
   */
  const getSecondArgumentAsString = () => {
    ...
    <div className='App'>
      <div className='calc'>
        <div className='calc__input_container'>
          <input data-testid="first-element" id='first' className='calc__input' value={first} onChange={inputHandler}/>
          <label className='calc__input'>{sign}</label>
          <input data-testid='second-element' id='second' className='calc__input'value={second} onChange={inputHandler}/>
          <span>=</span>
          <label data-testid='result' className='calc__result'>{result}</label>
        </div>
        <div className='calc__btn_container'>
          {Object.entries(btns).map((k)=>(
            <Button key={k[0]} variant='outlined' onClick={k[1]}>{k[0]}</Button>
          ))}
        </div>
      </div>
    </div>
